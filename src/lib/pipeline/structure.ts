...
export async function structureContent(
  ...
  if (isAerialStyle) sectionCount = Math.max(sectionCount, 8);
    ...
    ["winding-roadmap", "linear-progression", "story-mountain"].includes(
      analysis.layout,
    )
  ) {
  ...
  if (
    ...
    if (coverage < 0.5 && coveredEntities.length >= 1) {
      console.log(
        `[Structure] Sparse data: ${coveredEntities.length}/${analysis.entities.length} entities covered. Reducing sections.`,
      );
...
`
  ...
  try {
    response = await geminiGenerate(
      PRO_MODEL,
      `Structure this content for a ${analysis.layout} infographic in ${analysis.style} style.

${intentInstruction}
${textPreservation}
Create ${sectionCount} sections${isEntityBased && hasEntities ? ` (one per entity: ${analysis.entities.join(", ")})` : " (increase if needed to cover all items)"}. Each section needs: ${isExecutiveStyle ? 'heading (title case, 4-8 words — a COMPLETE, punchy McKinsey action-title stating the KEY finding. It MUST be a complete phrase — NEVER a cut-off sentence, NEVER ending in "...". Good: "Spain commands 45% of olive oil". Bad: "The global market is projected to reach a...")' : "heading (CAPS, max 5 words, complete phrase, no ellipsis)"}, key_concept (one sentence), content (bullet points), visual_element (what to illustrate), labels (callout text).
...
${content.slice(0, 8000)}`,
    ...
    console.log(
      `[Structure] Using ${PRO_MODEL} for executive-grade structuring`,
    );
  } catch (proError) {
    console.warn(
      `[Structure] ${PRO_MODEL} failed, falling back to ${TEXT_MODEL}:`,
      proError instanceof Error ? proError.message : proError,
    );
    response = await geminiGenerate(
      TEXT_MODEL,
      `Structure this content for a ${analysis.layout} infographic in ${analysis.style} style.
...
${content.slice(0, 6000)}`,
  ...
  try {
    ...
    if (!jsonMatch) {
      console.error(
        `[Structure] No JSON found in response. First 500 chars: ${response.slice(0, 500)}`,
      );
    }
    const parsed = JSON.parse(jsonMatch?.[0] || "{}");
    console.log(
      `[Structure] Parsed: ${parsed.sections?.length || 0} sections, title="${(parsed.title || "").slice(0, 50)}"`,
    );
    ...
    const validSections = sections.filter((s: any) => {
      ...
      const allPlaceholder =
        ...
        s.content.every(
          (c: string) =>
            c.toLowerCase().includes("no specific") ||
            c.toLowerCase().includes("data not available") ||
            c.toLowerCase().includes("no data") ||
            c.toLowerCase().includes("n/a"),
        );
      return !allPlaceholder;
    });
    console.log(
      `[Structure] ${sections.length} raw sections -> ${validSections.length} valid sections`,
    );
    ...
    console.error(
      `[Structure] Raw response (first 500 chars): ${response.slice(0, 500)}`,
    );
    return {
      title: "Infographic",
      subtitle: "",
...
If everything is fine: {"corrections": [], "risk_words": [], "fact_flags": [], "score": 100}`;
  ...
  try {
    ...
    if (!jsonMatch) {
      console.error(
        "[Compliance] No JSON found in response:",
    ...
    const corrections: ComplianceReport["corrections"] = (
      parsed.corrections || []
    ).map((c: any) => ({
      field: c.path || "",
      original: c.original || "",
