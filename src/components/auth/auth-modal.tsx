    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader className="items-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-(--z-gold)/10 mb-1">
            <Sparkles className="h-5 w-5 text-(--z-gold)" />
          </div>
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {error && (
              <p className="text-xs text-(--z-brick)">{error}</p>
            )}

            <Button
              type="submit"
              className="w-full bg-(--z-gold) hover:bg-(--z-gold)/90 text-white"
              disabled={loading}
            >
              {loading ? (
            ...
            {mode === 'signin' ? (
              <>
                ...
                <button
                  ...
                  className="text-(--z-gold) hover:underline font-medium"
                >
                  Sign up
                </button>
              </>
